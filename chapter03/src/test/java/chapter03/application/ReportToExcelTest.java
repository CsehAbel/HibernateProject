...
//        List<String> history_iug = NativeQueryTest.createListFromHistory(param);
        ...
        if (notInPolicy.size() > 0){
            writeExcelToFile(notInPolicy,"fokus_notInPolicy_%d.xlsx",path);
        }
        List<Fwpolicy> notInRuleset = allthree.notInRlst;
        if (notInRuleset.size() > 0){
            writeExcelToFile(notInRuleset,"fokus_notInRuleset_%d.xlsx",path);
        }
        ...
        try {
            writeUnionToJsonFile(report2, "fokus_bp_%d.json",path);
